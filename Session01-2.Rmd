...
```{r}
subset <- data %>% 
  select(gene, Samples, FPKM)### Modify this code block by usung the fucntion select
...
```{r}
subset_2 <- data %>%
  filter(FPKM >= 200, characteristics_ch1 == 'tissue: normal breast tissue')
size_not_cancer_abundant <- subset_2 %>% dim()
...
```{r}
BCRAmean <- data %>% filter(gene == 'BRCA1') %>% summarize(average_exp = mean(FPKM))### pipe your commands here
```
...
In the code block below, select BRCA1 and BRCA2, calculate the average expression per gene, per tissue per metastasis state.

```{r}
data %>% 
  filter( gene == 'BRCA1' | gene == 'BRCA2') %>%
  group_by(metastasis.ch1, gene, tissue.ch1) %>%
